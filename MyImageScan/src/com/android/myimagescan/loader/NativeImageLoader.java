package com.android.myimagescan.loader;
...
public class NativeImageLoader {
	...
	/**
	 * 此方法用来加载本地图片，mPoint封装了ImageView的宽和高，我们会根据ImageView控件的大小来裁剪Bitmap
	 * 如果你不想裁剪图片，调用loadNativeImage(final String path,final NativeImageCallBack mCallBack)来加载， 此处用到重载。
	 * @param path
	 * @param mPoint
	 * @param mCallBack
	 * @return
	...
	/**
	 * 根据View(主要是ImageView)的宽和高来计算Bitmap缩放比例。默认不缩放(比例为1) 
	 * @param options
	 * @param viewWidth
	 * @param viewHeight
	 * @return
	...
	private int computeScale(Options options, int viewWidth, int viewHeight) {
		...
		//加入Bitmap的宽度和高度大于我们设定图片的View的宽高，则计算缩放比例
		if(bitmapWidth > viewWidth || bitmapHeight > bitmapHeight){
	...
	/**
	 * 加载本地图片的回调接口，调用此接口会调用实现此接口的子类中的接口实现方法
	 * @author Administrator
	 *
	 */
